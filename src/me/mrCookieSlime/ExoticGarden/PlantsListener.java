...
public class PlantsListener implements Listener {
			...
			for (Berry berry: ExoticGarden.berries) {
				if (item.getID().equalsIgnoreCase(berry.toBush())) {
					switch(berry.getType()) {
						case BUSH: {
							e.getLocation().getBlock().setType(Material.LEGACY_LEAVES);
						...
						case DOUBLE_PLANT: {
							...
							e.getLocation().getBlock().getRelative(BlockFace.UP).setType(Material.LEGACY_SKULL);
						...
						default: {
							e.getLocation().getBlock().setType(Material.LEGACY_SKULL);
					...
					e.getWorld().playEffect(e.getLocation(), Effect.STEP_SOUND, Material.LEGACY_LEAVES);
					break;
				}
			}
		}
		...
		if (!cfg.getStringList("world-blacklist").contains(e.getWorld().getName())) {
			if (CSCoreLib.randomizer().nextInt(100) < cfg.getInt("chances.TREE")) {
				...
				for (y = e.getWorld().getMaxHeight(); y > 30; y--) {
					...
					if (!current.getType().isSolid() && current.getType() != Material.LEGACY_STATIONARY_WATER && berry.isSoil(current.getRelative(BlockFace.DOWN).getType())) {
						BlockStorage.store(current, berry.getItem());
						switch(berry.getType()) {
							case BUSH: {
								current.setType(Material.LEGACY_LEAVES);
								current.setData(berry.getData().toByte());
								break;
							}
							case FRUIT: {
								current.setType(Material.LEGACY_SKULL);
							...
							case DOUBLE_PLANT: {
								...
								current.getRelative(BlockFace.UP).setType(Material.LEGACY_SKULL);
				...
				for (y = e.getWorld().getMaxHeight(); y > 30; y--) {
					Block current = e.getWorld().getBlockAt(x, y, z);
					if (!current.getType().isSolid() && current.getType() != Material.LEGACY_STATIONARY_WATER && tree.isSoil(current.getRelative(0, -1, 0).getType())) {
						flat = true;
						for (int i = 0; i < 5; i++) {
							for (int j = 0; j < 5; j++) {
								for (int k = 0; k < 6; k++) {
	...
	public void onHarvest(BlockBreakEvent e) {
		if (CSCoreLib.getLib().getProtectionManager().canBuild(e.getPlayer().getUniqueId(), e.getBlock())) {
			if (e.getBlock().getType().equals(Material.LEGACY_SKULL)) dropFruitFromTree(e.getBlock());
			if (e.getBlock().getType().equals(Material.LEGACY_LEAVES) || e.getBlock().getType().equals(Material.LEGACY_LEAVES_2)) dropFruitFromTree(e.getBlock());
			if (e.getBlock().getType() == Material.LEGACY_LONG_GRASS) {
	...
	public void onInteract(PlayerInteractEvent e) {
		...
		if (CSCoreLib.getLib().getProtectionManager().canBuild(e.getPlayer().getUniqueId(), e.getClickedBlock())) {
			...
			if (item != null ) {
				e.getClickedBlock().getWorld().playEffect(e.getClickedBlock().getLocation(), Effect.STEP_SOUND, Material.LEGACY_LEAVES);
					...
					if (check != null) {
						for (Tree tree: ExoticGarden.trees) {
							if (check.getID().equalsIgnoreCase(tree.fruit)) {
								...
								drop.getWorld().playEffect(drop.getLocation(), Effect.STEP_SOUND, Material.LEGACY_LEAVES);
